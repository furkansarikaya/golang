...
// Dosyaları TAR dosyası olarak sıkıştırmak.
// TAR dosyası üretmek
...
func createArchiveTARFile(archiveFileName string) int {
	...
	for _, fileMame := range files {
		if err := addFile(fileMame, tw); err != nil {
			log.Fatalf("%s dosyası TAR dosyasına eklenirken hata meydana geldi : %s", fileMame, err)
		}
	}
	return 1
}
